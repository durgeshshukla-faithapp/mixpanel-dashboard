...
export async function getReports() {
  const rows = await readRange('Reports!A2:K1000');
  return rows
    .map((r, i) => ({
      row: i + 2,
      name: r[0] || `Report ${i + 2}`,
      ...
      slackFormat: (r[8] || '').trim().toLowerCase(),
      slackGroup: (r[9] || '').trim().toLowerCase(),
      // Goals: JSON object e.g. {"Sum of value": 100000, "Uniques": 50}
      goals: (() => { try { return JSON.parse(r[10] || '{}'); } catch { return {}; } })(),
    }))
    .filter((r) => r.link);
}
...
// ── Funnel snapshot cache ────────────────────────────────────────────────────
// Stores yesterday's funnel values so today's can be compared.
// Uses a "FunnelCache" tab with rows: ReportRow | Date | MetricKey | Value

async function ensureFunnelCacheTab(sheets) {
  try {
    await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.SHEET_ID,
      range: 'FunnelCache!A1',
    });
  } catch {
    await sheets.spreadsheets.batchUpdate({
      spreadsheetId: process.env.SHEET_ID,
      requestBody: { requests: [{ addSheet: { properties: { title: 'FunnelCache' } } }] },
    });
    await sheets.spreadsheets.values.update({
      spreadsheetId: process.env.SHEET_ID,
      range: 'FunnelCache!A1',
      valueInputOption: 'RAW',
      requestBody: { values: [['ReportRow', 'Date', 'MetricKey', 'Value']] },
    });
  }
}

// Saves today's funnel snapshot for a report
export async function saveFunnelSnapshot(reportRow, todayIso, results) {
  const auth = getAuth();
  const sheets = google.sheets({ version: 'v4', auth });
  await ensureFunnelCacheTab(sheets);

  // Read existing cache, filter out this report's rows for today, then append fresh
  const existing = await readRange('FunnelCache!A2:D100000');
  const rowStr = String(reportRow);
  const kept = existing.filter((r) => !(String(r[0]) === rowStr && r[1] === todayIso));

  const newRows = Object.entries(results).map(([metricKey, metricData]) => {
    const val = metricData?.rows?.[0]?.[0];
    return [reportRow, todayIso, metricKey, val ?? ''];
  });
  ...
  await sheets.spreadsheets.values.clear({
    spreadsheetId: process.env.SHEET_ID,
    range: 'FunnelCache!A2:D100000',
  });
  if (allRows.length > 0) {
    await sheets.spreadsheets.values.update({
      spreadsheetId: process.env.SHEET_ID,
      range: 'FunnelCache!A2',
      valueInputOption: 'RAW',
      requestBody: { values: allRows },
    });
  }
...
// Reads the most recent snapshot BEFORE todayIso for a report
export async function getFunnelBaseline(reportRow, todayIso) {
  try {
    const rows = await readRange('FunnelCache!A2:D100000');
    const rowStr = String(reportRow);
    const filtered = rows.filter((r) => String(r[0]) === rowStr && r[1] < todayIso);
    if (!filtered.length) return null;

    // Find most recent date
    const dates = [...new Set(filtered.map((r) => r[1]))].sort();
    const mostRecent = dates[dates.length - 1];
    const baseline = {};
    filtered
      .filter((r) => r[1] === mostRecent)
      .forEach((r) => { baseline[r[2]] = Number(r[3]) || 0; });
    return { date: mostRecent, values: baseline };
  } catch {
    return null;
  }
}
