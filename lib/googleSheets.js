...
// Col E = (optional) Postgres Query, Col F = (optional) Postgres Label,
// Col G = (optional) Owner, Col H = (optional) Funnel Link,
// Col I = Slack Notify (yes/y/1 = send to Slack, anything else = skip),
// Col J = Slack Metrics (comma-separated metric name fragments, or ALL/blank = all metrics)
export async function getReports() {
  ...
  return rows
    .map((r, i) => ({
      ...
      owner: (r[6] || '').trim(),
      funnelLink: (r[7] || '').trim(),
      slackNotify: /^(yes|y|1)$/i.test((r[8] || '').trim()),
      slackMetrics: (r[9] || '').trim() || 'ALL',
    }))
    .filter((r) => r.link);
}
