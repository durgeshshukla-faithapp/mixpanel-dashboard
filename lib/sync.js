...
export async function performSync() {
  ...
  for (const report of reports) {
    ...
    try {
      ...
      console.log(`[sync] ${report.name}: topKeys=[${topKeys.join(',')}] hasSeries=${!!raw?.series} hasResults=${!!raw?.results}`);
      if (raw?.series) {
        const mk = Object.keys(raw.series)[0];
        if (mk) {
          const bk = Object.keys(raw.series[mk] || {}).slice(0,2);
          console.log(`[sync] ${report.name}: first metric="${mk}" breakdown sample=[${bk.join(',')}]`);
        }
      }
