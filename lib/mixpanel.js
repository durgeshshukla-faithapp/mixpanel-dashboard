...
// different shape (like Funnel "Conversion Rate" columns). Logs the raw shape once per
// metric if nothing recognizable is found, so it can be seen in Vercel logs and fixed.
const loggedUnknownShapes = new Set();
function extractMetricValue(rawVal, metricName) {
  ...
  if (!loggedUnknownShapes.has(metricName)) {
    loggedUnknownShapes.add(metricName);
    console.error(
      `[mixpanel] Unrecognized value shape for metric "${metricName}": ${JSON.stringify(rawVal)}`
    );
  }
  return 0;
}
  ...
  Object.keys(series).forEach((metricName) => {
    ...
    Object.keys(bySource).forEach((source) => {
      ...
      Object.keys(byDate).forEach((dateStr) => {
        ...
        const value = extractMetricValue(rawVal, metricName);
    ...
    // If there was no real breakdown (e.g. a filtered/single-segment report), Mixpanel
    // puts everything under "$overall" instead of named sources. Use that as a fallback
    // "Overall" source rather than silently discarding all the data.
    if (sourcesSet.size === 0 && bySource['$overall']) {
      const byDate = bySource['$overall'];
      Object.keys(byDate).forEach((dateStr) => {
        if (dateStr === '$overall') return;
        const value = extractMetricValue(byDate[dateStr], metricName);
        if (isNaN(value)) return;
        const isoDate = toIsoDate(dateStr);
        if (!isoDate) return;
