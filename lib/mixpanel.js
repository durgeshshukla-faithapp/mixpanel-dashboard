...
// Fetches an A/B report using the /query/report endpoint which returns
// { results: { metricName: { rows: [[variant, value], ...] } } } format
// instead of the time-series { series: {...} } format
export async function fetchMixpanelABReport(reportId) {
  ...
  const endpoint =
    `https://mixpanel.com/api/query/report?project_id=${projectId}&bookmark_id=${reportId}`;
  ...
  if (!res.ok) {
    // Fallback to insights endpoint if report endpoint fails
    const today = new Date();
    const toDate = today.toISOString().slice(0, 10);
    const fromDateObj = new Date(today);
    fromDateObj.setDate(fromDateObj.getDate() - 90);
    ...
    const fallback = await fetch(
      ...
      { headers: { accept: 'application/json', authorization: `Basic ${authToken.trim()}` }, cache: 'no-store' }
    );
    if (!fallback.ok) throw new Error(`Mixpanel AB report ${res.status}`);
