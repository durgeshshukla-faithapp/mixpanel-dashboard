...
// Fetches a report using the Mixpanel bookmarks endpoint which returns
// { results: { metricName: { rows: [[value]] } } } format — used for
// A/B test reports and funnel-snapshot reports (single value per metric, no dates)
export async function fetchMixpanelABReport(reportId) {
  const authToken = process.env.MIXPANEL_AUTH_TOKEN;
  const projectId = process.env.MIXPANEL_PROJECT_ID;
  if (!authToken) throw new Error('MIXPANEL_AUTH_TOKEN not set');

  // Try the bookmarks/query endpoint first (returns results format directly)
  const endpoints = [
    `https://mixpanel.com/api/app/projects/${projectId}/bookmarks/${reportId}/query`,
    `https://mixpanel.com/api/2.0/insights?project_id=${projectId}&bookmark_id=${reportId}`,
  ];
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      // If response has results key with metric data, use it
      if (data?.results && typeof data.results === 'object' && !Array.isArray(data.results)) {
        const firstVal = Object.values(data.results)[0];
        if (firstVal?.rows || firstVal?.headers) return data.results;
      }
      // If response itself looks like results (keys map to {rows: [...]} objects)
      if (typeof data === 'object' && !data.series && !data.headers) {
        const firstVal = Object.values(data)[0];
        if (firstVal?.rows) return data;
      }
    } catch (e) {
      continue;
    }
  ...
  // Final fallback: use the insights API which gives series format,
  // then extract from series.$overall or sum the values
  const today = new Date();
  const toDate = today.toISOString().slice(0, 10);
  const fromDateObj = new Date(today);
  fromDateObj.setDate(fromDateObj.getDate() - 90);
  ...
  const res = await fetch(
    `https://mixpanel.com/api/query/insights?project_id=${projectId}&bookmark_id=${reportId}&from_date=${fromDate}&to_date=${toDate}`,
    {
      headers: { accept: 'application/json', authorization: `Basic ${authToken.trim()}` },
      cache: 'no-store',
    }
  ...
  if (!res.ok) throw new Error(`Mixpanel report fetch failed: ${res.status}`);
  const raw = await res.json();

  // Convert series format to results format
  // A/B reports: series[metric][breakdown][date] = value → results[metric].rows = [[breakdown, $overall_val]]
  // Funnel reports: series[metric][singleKey][date] = value → results[metric].rows = [[value]]
  const series = raw?.series || {};
  const results = {};

  Object.entries(series).forEach(([metricName, byBreakdown]) => {
    if (!byBreakdown || typeof byBreakdown !== 'object') {
      results[metricName] = { rows: [[null]] };
      return;
    }

    const breakdownKeys = Object.keys(byBreakdown);
    const isAB = breakdownKeys.some((k) =>
      /variant\s+[ab]|^\$?overall$|\bcontrol\b|\btreatment\b/i.test(k)
    );

    if (isAB) {
      // A/B format: preserve breakdown keys, use $overall value for each variant
      const rows = breakdownKeys.map((breakdown) => {
        const byDate = byBreakdown[breakdown];
        // Prefer $overall key, then last date, then first value
        let val = null;
        if (byDate && typeof byDate === 'object') {
          if ('$overall' in byDate) {
            const v = byDate['$overall'];
            val = typeof v === 'object' ? (v.all ?? v.value ?? null) : v;
          } else {
            // Sum or take last value across dates
            const dates = Object.keys(byDate).sort();
            const lastDate = dates[dates.length - 1];
            const v = byDate[lastDate];
            val = typeof v === 'object' ? (v.all ?? v.value ?? null) : v;
          }
        }
        return [breakdown, val != null ? Number(val) : null];
      });
      results[metricName] = { rows };
    } else {
      // Single-value funnel: extract scalar from first breakdown
      const firstBreakdown = byBreakdown[breakdownKeys[0]];
      let val = null;
      if (firstBreakdown && typeof firstBreakdown === 'object') {
        const vals = Object.values(firstBreakdown);
        if (vals.length > 0) {
          const v = vals[0];
          val = typeof v === 'object' ? (v.all ?? v.value ?? null) : v;
        }
      } else if (typeof firstBreakdown === 'number') {
        val = firstBreakdown;
      }
      results[metricName] = { rows: [[val]] };
    }
  });
