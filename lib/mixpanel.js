...
export async function fetchMixpanelABReport(reportId) {
  ...
  // Convert series format to results format for single-value reports
  // series[metricName][$overall] = value OR series[metricName][date] = value
  const series = raw?.series || {};
  const results = {};
  Object.entries(series).forEach(([metricName, byBreakdown]) => {
    // For single-value metrics, all breakdowns have same value — use first non-null
    if (typeof byBreakdown === 'object') {
      const vals = Object.values(byBreakdown);
      if (vals.length === 0) { results[metricName] = { rows: [[null]] }; return; }
      // Check if it's {$overall: value} or {date: value} pattern
      const firstVal = vals[0];
      if (typeof firstVal === 'number') {
        // Single-value: use it directly
        results[metricName] = { rows: [[firstVal]] };
      } else if (typeof firstVal === 'object' && firstVal !== null) {
        // Could be {all: N} or similar
        const n = firstVal.all ?? firstVal.value ?? Object.values(firstVal)[0] ?? null;
        results[metricName] = { rows: [[n]] };
      } else {
        results[metricName] = { rows: [[null]] };
      }
    } else {
      results[metricName] = { rows: [[byBreakdown]] };
    }
  });
