...
function parseDate(str) {
  ...
  // Already ISO date: "2026-07-21"
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;
  // ISO datetime with timezone: "2020-08-17T00:00:00-07:00" → "2020-08-17"
  const isoMatch = s.match(/^(\d{4}-\d{2}-\d{2})T/);
  if (isoMatch) return isoMatch[1];
  // "Jul 21, 2026"
  const humanMatch = s.match(/^(\w{3})\s+(\d{1,2}),\s+(\d{4})$/);
  if (humanMatch && MONTHS[humanMatch[1]]) {
    return `${humanMatch[3]}-${MONTHS[humanMatch[1]]}-${humanMatch[2].padStart(2,'0')}`;
  }
  return null;
}
...
function buildFromSeries(series, todayIso) {
  ...
  Object.entries(series).forEach(([metricKey, byBreakdown]) => {
    ...
    const breakdownKeys = Object.keys(byBreakdown).filter(k => k !== '$overall');
    if (!breakdownKeys.length) return;

    // Skip A/B reports
    if (isABVariant(breakdownKeys[0])) return;

    // Check if breakdown keys are dates directly
    // Format from /query/insights: series[metric][date] = { all: value }
    // OR: series[metric][source][date] = { all: value }
    const firstKey = breakdownKeys[0];

    if (isDate(firstKey)) {
      // Breakdown keys ARE dates → simple date-series, no source breakdown
      const dateMap = {};
      breakdownKeys.forEach((dateStr) => {
        const iso = parseDate(dateStr);
        if (!iso) return;
        const val = byBreakdown[dateStr];
        const n = typeof val === 'object'
          ? (val.all ?? val.value ?? Object.values(val)[0] ?? 0)
          : Number(val) || 0;
        dateMap[iso] = n;
      });
      const dates = Object.keys(dateMap).sort();
    ...
    // Breakdown keys are SOURCES → each source has { date: {all: value} } map
    const sourceMap = {};
    const allDates = new Set();

    breakdownKeys.forEach((source) => {
      const byDate = byBreakdown[source];
