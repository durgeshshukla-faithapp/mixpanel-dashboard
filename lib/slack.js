  ...
  const metricBlocks = shown.map((s) => {
    ...
    if (thin) {
      ...
      if (isMeaningful(s)) {
        ...
        lines.push(flat
          ? `${emoji(d7)} About the same as a normal day (normally around ${fmtValue(s.avg7d, s.label)})`
          : `${emoji(d7)} ${pctWord} a normal day (normally around ${fmtValue(s.avg7d, s.label)})`);
      } else {
        // Numbers this small swing wildly day to day; a percentage would overstate it.
        lines.push(`Normally around ${fmtValue(s.avg7d, s.label)} a day. Numbers this small move around a lot, so day-to-day changes are usually not significant.`);
      }
      ...
      if (d30 != null && isMeaningful(s)) {
        ...
        if (Math.abs(d30) >= 3) {
          lines.push(`Over the last 30 days it usually sits near ${fmtValue(s.avg30d, s.label)} — today is ${Math.abs(d30).toFixed(0)}% ${dir30} that.`);
        } else {
          lines.push(`That is also in line with the last 30 days (usually near ${fmtValue(s.avg30d, s.label)}).`);
        }
      }
