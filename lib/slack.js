...
export function buildFunnelComparisonBlocks(groupTitle, funnelsData) {
  ...
  // Use FULL metric keys (including A./B. prefix) to avoid duplicate label collision
  // e.g. "A. Uniques of Overall PV" and "B. Uniques of Overall PV" stay distinct
  const keyOrder = [];
  const keySeen = new Set();
  funnelsData.forEach((f) => {
    Object.keys(f.results).forEach((key) => {
      if (!keySeen.has(key)) { keySeen.add(key); keyOrder.push(key); }
    });
  });

  // Display label: strip prefix but keep letter if label would duplicate
  const labelCount = {};
  keyOrder.forEach((key) => {
    const bare = key.replace(/^[A-Z]\.\s*/, '').trim();
    labelCount[bare] = (labelCount[bare] || 0) + 1;
  });
  function displayLabel(key) {
    const bare = key.replace(/^[A-Z]\.\s*/, '').trim();
    if (labelCount[bare] > 1) return key.trim(); // keep "A. Uniques..." distinct
    return bare;
  }
  ...
  const dataRows = keyOrder.map((key) => {
    const label = displayLabel(key);
    const cells = funnelsData.map((f) => {
      const entry = f.results[key];
      const val = entry ? extractValue(entry) : null;
      return pad(fmt(val), colW);
    });
