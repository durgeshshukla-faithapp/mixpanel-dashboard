...
// ── A/B Test format ────────────────────────────────────────────────────────
// Handles breakdown patterns like "Variant A, $overall", "Variant B, $overall"
// Prefers "$overall" rows for each variant to get the aggregate value
export function buildABBlocks(reportName, rawResults) {
  ...
  // Gets the $overall value for a variant (e.g. "Variant A" or "A")
  function getVariantValue(rows, variantLabel) {
    // Try "Variant X, $overall" format first
    ...
    const exactRow = rows.find((r) =>
      String(r[0]).toLowerCase().trim() === variantLabel.toLowerCase()
    );
    if (exactRow) return Number(exactRow[1]);

    return null;
  }

  // Auto-detect variant labels from breakdown values
  function detectVariants(rows) {
    const allKeys = rows.map((r) => String(r[0]));
    // Check for "Variant A/B" pattern
    if (allKeys.some((k) => /variant a/i.test(k))) return ['Variant A', 'Variant B'];
    // Check for plain "A/B" pattern
    if (allKeys.some((k) => /^a$/i.test(k))) return ['A', 'B'];
    // Check for Control/Treatment
    if (allKeys.some((k) => /control/i.test(k))) return ['Control', 'Treatment'];
    return ['A', 'B'];
  }

  const firstMetric = Object.values(rawResults)[0];
  const firstRows = firstMetric?.rows || [];
  ...
  const lines = Object.entries(rawResults).map(([metricKey, metricData]) => {
    ...
    // Winner first (higher = better for most metrics)
