...
// ── A/B Test format ────────────────────────────────────────────────────────
// For reports where breakdown is A/B (not dates).
// Shows each metric as "MetricName — B X% vs A Y%" (winner first)
export function buildABBlocks(reportName, results) {
  if (!results) return [];
  ...
  const lines = Object.entries(results).map(([metricKey, metricData]) => {
    ...
    const aRow = rows.find((r) => r[0] === 'A');
    const bRow = rows.find((r) => r[0] === 'B');
    const aVal = aRow ? Number(aRow[1]) : null;
    const bVal = bRow ? Number(bRow[1]) : null;

    function fmt(v) {
      if (v == null) return '—';
      if (v !== 0 && Math.abs(v) < 1) return (v * 100).toFixed(2) + '%';
      return fmtNum(v);
    }
    ...
    const winner = (bVal != null && aVal != null && bVal > aVal) ? `${bFmt} vs ${aFmt}` : `${aFmt} vs ${bFmt}`;
    return `• *${label}* — ${winner}`;
  });
