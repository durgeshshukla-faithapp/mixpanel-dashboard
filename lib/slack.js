...
export function computeStats(matrices, todayIso, slackMetrics = 'ALL', goals = {}) {
  ...
  allowedKeys.forEach((metricKey) => {
    ...
    stats[metricKey] = {
      label, today, avg7d, avg30d, bySource, sourceBaselines,
...
export function buildSummaryBlocks(dashboardName, stats, todayIso) {
  ...
  const metricBlocks = shown.map((s) => {
    ...
    const lines = [`*${s.label}*`, `Today: *${fmtNum(s.today)}*`];
    ...
    if (thin) {
      ...
      if (isMeaningful(s)) {
        ...
        lines.push(flat
          ? `${emoji(d7)} About the same as a normal day (normally around ${fmtNum(Math.round(s.avg7d))})`
          : `${emoji(d7)} ${pctWord} a normal day (normally around ${fmtNum(Math.round(s.avg7d))})`);
      } else {
        // Numbers this small swing wildly day to day; a percentage would overstate it.
        lines.push(`Normally around ${fmtNum(Math.round(s.avg7d))} a day. Numbers this small move around a lot, so day-to-day changes are usually not significant.`);
      }
      ...
      if (d30 != null && isMeaningful(s)) {
        ...
        if (Math.abs(d30) >= 3) {
          lines.push(`Over the last 30 days it usually sits near ${fmtNum(Math.round(s.avg30d))} — today is ${Math.abs(d30).toFixed(0)}% ${dir30} that.`);
        } else {
          lines.push(`That is also in line with the last 30 days (usually near ${fmtNum(Math.round(s.avg30d))}).`);
        }
      }
  ...
  const headerNote = [];
  if (flagged.length > 0) {
    const names = flagged.map((s) => s.label).join(', ');
