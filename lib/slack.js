...
export function buildFunnelComparisonBlocks(groupTitle, funnelsData) {
  ...
  // Collect all unique step labels across all funnels (in order of first appearance)
  const labelOrder = [];
  ...
  // Header row: Step | Funnel1 | Funnel2 ...
  const shortNames = funnelsData.map((f) => {
    // Shorten "CHOOSE SLOT FUNNEL" → "CHOOSE SLOT", etc.
    return f.name.replace(/\s*FUNNEL\s*$/i, '').slice(0, colW - 1);
  });
  const headerRow = pad('Step', stepW) + shortNames.map((n) => pad(n, colW)).join('');

  // Data rows: each label × each funnel's value
  const dataRows = labelOrder.map((label) => {
    const cells = funnelsData.map((f) => {
      // Find matching metric by label
      const entry = Object.entries(f.results).find(([key]) =>
        key.replace(/^[A-Z]\.\s*/, '').trim() === label
      );
      const val = entry ? extractValue(entry[1]) : null;
  ...
  return [
    {
      type: 'header',
      text: { type: 'plain_text', text: `📊 ${groupTitle} · ${now}`, emoji: true },
    },
    {
      type: 'section',
